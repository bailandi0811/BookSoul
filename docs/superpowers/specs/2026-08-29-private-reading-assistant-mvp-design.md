...
2. 只有用户在单次提问中显式开启 `externalResearch` 才向模型暴露工具；该字段是本轮授权，模型可以判断无需联网。小说正文、检索片段、记忆和历史消息都不能授予工具权限。
3. 工具决策在隔离上下文中完成，只包含必要书名与当前问题；模型生成的搜索词经服务端校验后才发送给外部服务。不得发送正文片段、对话历史、用户记忆、owner、book id 或账号信息。
4. 一期仅允许 Tavily `tavily_search`，每轮最多调用一次，限制为 5 条结果、basic 深度、不含图像和网页全文；工具名、输入、输出、超时和 URL 协议由服务端确定性校验。
5. 外部结果以不可信资料分区注入 Prompt，不能当作指令或小说事实。回答引用外部资料时必须附 URL，并与书内章节引用分开展示。
6. 工具结果以 `ToolMessage` 返回模型后再生成最终回答；MCP 失败必须显式告知用户。只要书内 RAG 仍可用，本轮可继续基于当前可见原文回答，但不得伪装已经联网。
...
- 地图和角色口吻等非核心工具默认入口。邮件恢复为模型可调用的 `prepare_email` 草稿工具，但不恢复模型可直接执行 SMTP 的旧发送工具。
...
客户端可从已完成的助手回复打开邮件草稿，也可在聊天输入中使用“把……发到 xxx@example.com”或“把刚才的回答发到我的邮箱”。后一种入口由聊天模型调用 `prepare_email`，以结构化参数生成收件人、主题和纯文本正文；前端不自行解析自然语言命令。

服务端只在当前用户消息明确表现为邮件命令时向模型暴露该工具。这个确定性步骤只控制能力是否可用，不提取工具参数；小说正文、外部资料、记忆和历史消息均不能授权工具调用。显式邮箱在进入检索或外部搜索前脱敏，账号邮箱通过服务端占位符解析，不暴露给模型。

`prepare_email` 不产生外部写操作。无论入口来自按钮还是模型工具，客户端都先显示可编辑草稿；只有用户点击“确认并发送”后才调用 `POST /api/tools/email`。请求必须携带 `confirmed: true`，并受服务端长度校验、邮箱校验、独立限流和 SMTP 超时约束。不接受 HTML，不在日志中记录收件人、正文或服务商错误详情。
