...
let RagService = RagService_1 = class RagService {
    ...
    constructor(configService, milvusService, mcpService, toolsService) {
    ...
    async retrieveRelevantContent(question, k = 3) {
        try {
    ...
    async generateResponseStream(question, context, res, character = 'assistant') {
        ...
        try {
            ...
            if (tools.length > 0) {
                ...
                if (response.tool_calls && response.tool_calls.length > 0) {
                    this.logger.log(`Tool calls detected: ${response.tool_calls.map((t) => t.name).join(', ')}`);
