...
import type { AgentState, IntentClassification, IntentType } from '../state';
...
function detectNovelEntities(query: string): string[] {
...
function classifyByPattern(query: string): IntentType | null {
  const q = query.trim();
...
function isToolIntent(query: string): boolean {
...
function isComplexReasoningQuestion(query: string): boolean {
  return COMPLEX_REASONING_PATTERNS.some((k) => query.includes(k));
}

function isSimpleDirectQuestion(query: string): boolean {
  const q = query.trim();
  if (!q) return false;
  ...
  if (simplePatterns.some((k) => q.includes(k))) return true;
  if (isComplexReasoningQuestion(q)) return false;
  return q.length <= 18;
}
...
// ========== Create Classify Node ==========

export const createClassifyNode = (model: ChatOpenAI) => {
  return async (state: AgentState): Promise<Partial<AgentState>> => {
    ...
    try {
      classification = JSON.parse(llmResponse.content as string);
    } catch {
      // Fallback
      classification = {
        intent_type:
